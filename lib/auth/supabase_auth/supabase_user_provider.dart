...
class ManutecProjectSupabaseUser extends BaseAuthUser {
  ManutecProjectSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> manutecProjectSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ManutecProjectSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
